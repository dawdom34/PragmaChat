...
def account_view(request, *args, **kwargs):
	"""
	- Returns:
		is_self (boolean) - information about whether the user is looking at his profile 
			is_friend (boolean) - information about friendship
				-1: NO_REQUEST_SENT - information about friend request status
				0: THEM_SENT_TO_YOU
				1: YOU_SENT_TO_THEM
	"""
	context = {}
	...
	if account:
		...
		user = request.user
		# User is not looking at his own profile
		if user.is_authenticated and user != account:
			...
			if friends.filter(pk=user.id):
				...
				# User is not friend, check if any friend request was send
				is_friend = False
			...
			# User is looking at own profile, get any active friend request
			try:
